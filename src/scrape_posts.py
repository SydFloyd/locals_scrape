    ...
    async with aiohttp.ClientSession(cookies=session.cookies.get_dict(), headers=headers) as aio_sess:
        ...
        try:
            while True:
                ...
                for post in posts:
                    ...
                    if SCRAPE_COMMENTS:
                        try:
                            ...
                            post_data["comment_data"] = [{'comment_id': 'error', 'comment': str(e)}]
                ...
                # Save posts and page number periodically
            ...
            # Done scraping, wait for video tasks to complete
            if video_tasks:
                print(f"\nWaiting on {len(video_tasks)} video downloads...")
